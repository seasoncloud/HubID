...
count2 = count2[idx,good_genes]

batch_id2 = groupondist(location2,size = 5000)
batch_id2 = paste0("2.",batch_id2)
...
count3 = count3[idx,good_genes]

batch_id3 = groupondist(location3,size = 5000)
batch_id3 = paste0("3.",batch_id3)

total_batch = c(batch_id1,batch_id2,batch_id3)
total_count = rbind(count1,count2,count3)
total_location = rbind(location1,location2,location3)

row_sum = rowSums(total_count)
total_count = total_count/row_sum*max(row_sum)

start = Sys.time()
out = nmfspatial_batch(total_count, 20, location = total_location, batch = total_batch, lengthscale = 500, initial = 1, smallIter = 10, maxiter = 2000, tolerance = 1e-10, error_freq = 100)
stop = Sys.time()
out$time = stop - start
print(stop - start)
save(out, file = paste0("modelssaved/three_sample_f20_s5K_l500_norm.RData"))