...
### set path
sample_name = 'HuColonCa-FFPE-ImmuOnco-LH_VMSC02001_20220427'
PATH_TO_MODELS = f'./LH_VMSC02001/models/'
PATH_TO_PATIENT_DFS_PKL = "/gladstone/engelhardt/pelka-collaboration/"+str(sample_name)+"/baysor_20231207/baysor_res_merged/cell_by_gene.csv.gz"
PATH_TO_TUMOR_MARKER_FEATURES_PKL = f'./LH_VMSC02001/LH_VMSC02001_marker_features.pkl'
PATH_TO_COHORT_METADATA = "/gladstone/engelhardt/pelka-collaboration/"+str(sample_name)+"/baysor_20231207/baysor_res_merged/segmentation_cell_stats.csv.gz"
...
import pandas as pd
#sample_name = 'HuColonCa-FFPE-ImmuOnco-LH_VMSC02001_20220427'
anndata_path='~/MERFISH.h5ad'   ### generate anndata for the datset
adata_sub = sc.read_h5ad(anndata_path)

df = pd.DataFrame(adata_sub.X, index=adata_sub.obs_names, columns=adata_sub.var_names)
df.loc[:, 'is_tumor']=True
df = df.join(adata_sub.obsm['spatial'])


# frame the data as that in the SpatialLDA structure

N_IMMUNE_CLUSTERS = 12
patient_dfs=dict()
patient_dfs[sample_name]=df
markers=df.columns[0:492]
num_patients = 1
...
if os.path.exists(PATH_TO_TUMOR_MARKER_FEATURES_PKL):
  ...
  neighborhood_feature_fn = functools.partial(neighborhood_to_marker, 
                                              markers=markers)
  tumor_marker_features = featurize_tumors(patient_dfs2, 
...
path_to_train_model = '_'.join((f'{PATH_TO_MODELS}/MERFISH_training',
...
# train complete data
PATH_TO_COMPLETE_MODEL = '_'.join((f'{PATH_TO_MODELS}/MERFISH_complete',
                                f'penalty={DIFFERENCE_PENALTY}',
                                f'topics={N_TOPICS}',
                                f'trainfrac={TRAIN_SIZE_FRACTION}')) + '.pkl'
